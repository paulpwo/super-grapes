...
Super-Grapes is an Elementor-like page builder built on GrapesJS. It's a pnpm monorepo with two packages (`@super-grapes/core`, `@super-grapes/ui`) and a vanilla example. All editor UI is custom vanilla TypeScript + CSS — GrapesJS only provides the canvas, component model, and undo/redo.

## Commands

```bash
pnpm dev          # Start vanilla example at localhost:5173 (Vite)
pnpm build        # Build all packages
pnpm build:core   # Build only @super-grapes/core
pnpm build:ui     # Build only @super-grapes/ui
...
The editor initializes in two phases (see `examples/vanilla/src/main.ts`):
...
**`packages/core/`** — GrapesJS initialization, component types, block definitions
...
**`packages/ui/`** — Editor shell, panels, controls, dark theme
...
- `theme/`: CSS files with `--sg-*` custom properties. `index.css` imports `grapesjs/dist/css/grapes.min.css` first, then all theme overrides
...
### Vite Aliases (Dev Only)

`examples/vanilla/vite.config.ts` resolves `@super-grapes/core` and `@super-grapes/ui` directly to package source — no pre-build needed for development.

## Critical Conventions

- GrapesJS CSS must be imported before theme CSS (toolbar, badge, tools overlay depend on GrapesJS base styles)
...
See `docs/IMPLEMENTATION.md` for full feature inventory and `docs/ROADMAP.md` for QA checklist with known bugs (e.g., slider stepping, drag & drop verification needed).
